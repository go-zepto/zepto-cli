...
func (e *Engine) watching(root string) error {
	return filepath.Walk(root, func(path string, info os.FileInfo, err error) error {
		...
		// exclude tmp dir
		if e.isTmpDir(path) {
			e.watcherDebug("!exclude %s", e.Config.rel(path))
...
func (e *Engine) watchDir(path string) error {
	...
	e.watcherDebug("watching %s", e.Config.rel(path))
	...
	go func() {
		...
		if e.Config.Build.ExcludeUnchanged {
			...
			if err != nil {
				e.watcherDebug("error building checksum cache: %v", err)
			}
		}
...
func (e *Engine) watchNewDir(dir string, removeDir bool) {
	...
	if isHiddenDirectory(dir) || e.isExcludeDir(dir) {
		e.watcherDebug("!exclude %s", e.Config.rel(dir))
		return
	}
	if removeDir {
...
func (e *Engine) start() {
	...
	for {
		...
		case filename = <-e.eventCh:
			...
			e.mainLog(color.YellowString("%s has changed, reloading zepto...", e.Config.rel(filename)))
		case <-firstRunCh:
			// go down
			break
		}
